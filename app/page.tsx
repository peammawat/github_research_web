...
import { Search, Github, Star, Calendar, Loader2, AlertCircle, ExternalLink, Plus, Trash2, Users, CheckCircle2, XCircle, User } from 'lucide-react';
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 selection:bg-blue-500/30">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 text-lg">ค้นหาโปรเจกต์พร้อมระบบตรวจสอบรายละเอียดจาก Readme</p>
        </div>
          ...
          <form onSubmit={handleSearch} className="space-y-6">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-[2] relative group">
                ...
                <input
                  type="text"
                  placeholder="ค้นหาโปรเจกต์หลัก เช่น React, Tailwind..."
              ...
              <div className="flex-grow relative group">
                ...
                <input
                  type="text"
                  placeholder="Username / Org (ไม่บังคับ)"
                  className="w-full pl-12 pr-4 py-4 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 bg-slate-800/50 text-slate-100 placeholder:text-slate-500 transition-all shadow-inner"
                  value={username}
              ...
              >
                {isLoading ? <><Loader2 className="animate-spin w-5 h-5" /> ค้นหา...</> : <><Search className="w-5 h-5" /> ค้นหาและตรวจสอบ</>}
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold text-slate-400">ตรวจสอบความต้องการจาก Description / Readme</label>
                <button 
                  type="button"
                  onClick={addDetail}
                ...
                >
                  <Plus className="w-3.5 h-3.5" /> เพิ่มรายละเอียดตรวจสอบ
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {details.map((detail, index) => (
                  <div key={index} className="relative group animate-in slide-in-from-left-2 fade-in">
                    <input
                      type="text"
                      placeholder={`เช่น MIT License, SSR, Tiny size...`}
                      value={detail}
